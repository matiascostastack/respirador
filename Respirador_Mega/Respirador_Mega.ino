...
const int Pote = A0;                              // Se selecciona la entrada A0 para la lectura analogica del potenciometro
...
int Pulsador_Pantalla = 2;                // Seleccion de entrada para Pulsador navegar en pantallas
...
int Pulsador_Seleccion = 3;                // Seleccion de entrada para Pulsador navegar en pantallas
...
int Dt_LCD = 100;
unsigned long Tiempo_Motor = 0;
int Dt_Motor = 10;
unsigned long Tiempo_Plateau = 0;
int Dt_Plateau = 100;
int presion = 20;
int Tiempo_Pulsador_Marcha = 0;
int Dt_Marcha = 1;
...
void setup()
{
  ...
  Wire.onRequest(requestEvent); //Interrupción habilitada cuando el maestro envía bytes en la comunicación I2C
...
void loop()
{
  ...
  case 2:
    lcd.setCursor(0, 0);
    lcd.print("PMax [3-40mmH2O]"); // Escribimos el Mensaje en el LCD.
    lcd.setCursor(0, 1);
    lcd.print("SEL:"); // Escribimos el Mensaje en el LCD.
    lcd.setCursor(4, 1);
    lcd.print(Pote_Display); // Escribimos el Mensaje en el LCD.
  ...
  case 3:
    lcd.setCursor(0, 0);
    lcd.print("PEEP [0-10mmH2O]"); // Escribimos el Mensaje en el LCD.
    lcd.setCursor(0, 1);
    lcd.print("SEL:"); // Escribimos el Mensaje en el LCD.
    lcd.setCursor(4, 1);
    lcd.print(Pote_Display); // Escribimos el Mensaje en el LCD.
...
{
  //************************************************************************************************************************//
  // Comunicacion I2c entre Mega (Esclavo) y Uno (Maestro)                                                                  //
  // Datos Transmitidos: Velocidad de Inspiracion, Velocidad de Expiracion, Presion maxima y PEEP                           //
  //************************************************************************************************************************//
  //El maestro obtiene los datos que fueron configurados por el operador
  ...
  aux1 = (unsigned int)Velo_Inspiracion_Tx; // aux = 46689, Pega somente a parte inteira da variável float (0 - 65536)
  byte2 = aux1;                             // byte2 = 0B01100001, pega apenas os primeros 8 bits
  byte1 = (aux1 >> 8);                      // byte1 = 0B10110110, pega os 8 ultimos bits
  // Ajustando o número depois da vírgula
  Velo_Inspiracion_Tx -= aux1;              // Deixa apenas o número depois da vírgula
  Velo_Inspiracion_Tx *= 10000;             // Multiplica por 10k para pegar 4 dígitos após a vírgula
  aux1 = (unsigned int)Velo_Inspiracion_Tx; // Pega somente o valor antes da vírgula
  byte4 = aux1;                             // byte2 = 0B00101110, pega apenas os primeros 8 bits
  byte3 = (aux1 >> 8);                      // byte1 = 0B00100010, pega os 8 ultimos bits

  //Se envia velocidad de Expiracion
  Velo_Expiracion_Tx = Velo_Expiracion;
  aux2 = (unsigned int)Velo_Expiracion_Tx; // aux = 46689, Pega somente a parte inteira da variável float (0 - 65536)
  byte6 = aux2;                            // byte2 = 0B01100001, pega apenas os primeros 8 bits
  byte5 = (aux2 >> 8);                     // byte1 = 0B10110110, pega os 8 ultimos bits
  // Ajustando o número depois da vírgula
  Velo_Expiracion_Tx -= aux2;              // Deixa apenas o número depois da vírgula
  Velo_Expiracion_Tx *= 10000;             // Multiplica por 10k para pegar 4 dígitos após a vírgula
  aux2 = (unsigned int)Velo_Expiracion_Tx; // Pega somente o valor antes da vírgula
  byte8 = aux2;                            // byte2 = 0B00101110, pega apenas os primeros 8 bits
  byte7 = (aux2 >> 8);                     // byte1 = 0B00100010, pega os 8 ultimos bits

  //Se envia Presion MAxima de trabajo
  PMAX_Tx = PMAX_Seleccionado;
  aux3 = (unsigned int)PMAX_Tx; // aux = 46689, Pega somente a parte inteira da variável float (0 - 65536)
  byte10 = aux3;                // byte2 = 0B01100001, pega apenas os primeros 8 bits
  byte9 = (aux3 >> 8);          // byte1 = 0B10110110, pega os 8 ultimos bits
  // Ajustando o número depois da vírgula
  PMAX_Tx -= aux3;              // Deixa apenas o número depois da vírgula
  PMAX_Tx *= 10000;             // Multiplica por 10k para pegar 4 dígitos após a vírgula
  aux3 = (unsigned int)PMAX_Tx; // Pega somente o valor antes da vírgula
  byte12 = aux3;                // byte2 = 0B00101110, pega apenas os primeros 8 bits
  byte11 = (aux3 >> 8);         // byte1 = 0B00100010, pega os 8 ultimos bits

  //Se envia Presion PEEP
  PEEP_Tx = PEEP_Seleccionado;
  aux4 = (unsigned int)PEEP_Tx; // aux = 46689, Pega somente a parte inteira da variável float (0 - 65536)
  byte14 = aux4;                // byte2 = 0B01100001, pega apenas os primeros 8 bits
  byte13 = (aux4 >> 8);         // byte1 = 0B10110110, pega os 8 ultimos bits
  // Ajustando o número depois da vírgula
  PEEP_Tx -= aux4;              // Deixa apenas o número depois da vírgula
  PEEP_Tx *= 10000;             // Multiplica por 10k para pegar 4 dígitos após a vírgula
  aux4 = (unsigned int)PEEP_Tx; // Pega somente o valor antes da vírgula
  byte16 = aux4;                // byte2 = 0B00101110, pega apenas os primeros 8 bits
  byte15 = (aux4 >> 8);         // byte1 = 0B00100010, pega os 8 ultimos bits
